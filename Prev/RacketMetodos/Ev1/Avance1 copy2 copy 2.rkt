...
(define (escribir elemento escritura)
  (cond
    [(esCommentario? elemento)
     (begin
       (define comment-parts (regexp-match* #px"(?<=//| )[^/ ].*" elemento))
       (for-each (lambda (part)
                   (display "<text class='comment'>" escritura)
                   (display part escritura)
...
(define (escribir-lista lista)
  ...
  (let loop ((lst lista) (comment-found #f))
    (cond
      ...
      [(string=? (car lst) "<br>")
       (begin
         ...
         (newline escritura)
         (loop (cdr lst) comment-found))]
      [(and (not comment-found) (string-prefix? "//" (car lst)))
       (begin
         (display "<text class='comment'>" escritura)
         (display (string-join lst " ") escritura)
         (display "</text>" escritura)
         (newline escritura)
         (loop (cdr lst) #t))]
      [comment-found
